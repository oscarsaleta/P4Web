...
void WSphere::integrateOrbit(int dir)
{
    ...
    if (dir == 0) {
        ...
        study_->current_orbit->current_f_orbits->next_point =
            integrate_orbit(pcoord, study_->config_currentstep, dir, CORBIT,
                            study_->config_intpoints, &sep);
    ...
    if (study_->current_orbit->f_orbits == nullptr) {
        study_->current_orbit->f_orbits =
            integrate_orbit(pcoord, study_->config_step, dir, CORBIT,
                            study_->config_intpoints, &sep);
    } else {
        study_->current_orbit->current_f_orbits->next_point = new orbits_points;
        study_->current_orbit->current_f_orbits =
            study_->current_orbit->current_f_orbits->next_point;
...
void WSphere::drawOrbits()
{
    ...
    for (orbit = study_->first_orbit; orbit != nullptr;
